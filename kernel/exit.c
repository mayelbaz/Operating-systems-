...
asmlinkage long sys_wait4(pid_t pid,unsigned int * stat_addr, int options, struct rusage * ru)
{
	...
	/*Our Changes*/
	if (current != NULL && current->_enable_policy == 1 && current->_plevel < 2) {
		if (current->_log_arr != NULL) {
			int i = 0;
			while (current->_log_arr[i].syscall_req_level != -1){
				i++;
			}
			current->_log_arr[i].syscall_req_level = 2;
			current->_log_arr[i].proc_level = current->_plevel;
			current->_log_arr[i].time = jiffies;
		}
		return -EINVAL;
	}
